...
__global__ void reduceNeighbored(int *d_idata, int *d_odata, unsigned int size, unsigned char * traceArray, int numberOfThreads) {
   ...
   /* node 8 sync */
   __syncthreads();
   for(/* node 9 */ int stride = 1; /* node 10 */ stride < blockDim.x; /* node 14 */ stride *= 2) {
...
int main(int argc, char **argv) {
   ...
   /* node 10 */
   reduceNeighbored<<<gridsize, blocksize>>>(d_idata, d_odata, size, instTrace0, numberOfThreads0);
   /* node 11 */
   cudaDeviceSynchronize();
   /* node 12 */
