...
__global__ void findMin(int *g_data, int *g_odata, int N, unsigned char * traceArray, int numberOfThreads) {
   ...
   // __syncthreads(); // bug
   int s;
   for(/* node 5 */ s = blockDim.x / 2; /* node 6 */ s > 0; /* node 13 */ s /= 2) {
      registerTrace(traceArray, numberOfThreads, map, cont++, 7);
      if(tid < s && i + s < N) {
         registerTrace(traceArray, numberOfThreads, map, cont++, 8);
         if(sdata[tid] > sdata[tid + s]) { registerTrace(traceArray, numberOfThreads, map, cont++, 9); sdata[tid] = sdata[tid + s]; }
   ...
   if(tid == 0) { registerTrace(traceArray, numberOfThreads, map, cont++, 15); g_odata[blockIdx.x] = sdata[0]; }
