...
__global__ void multiMatrix(int *d_a, int *d_b, int *d_c, int N, unsigned char * traceArray, int numberOfThreads) {
   ...
   int tile_idx;
   for(/* node 2 */ tile_idx = 0; /* node 3 */ tile_idx < gridDim.x; /* node 23 */ tile_idx++) {
      ...
      if(idx < N && idy < N) {
         ...
         int i;
         for(/* node 15 */ i = 0; /* node 16 */ i < 16; /* node 20 */ i++) {
   ...
   registerTrace(traceArray, numberOfThreads, map, cont++, 24);
   __syncthreads();
   /* node 25 */
   if(idx < N && idy < N) {
