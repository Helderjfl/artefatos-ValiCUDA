...
int main(int argc, char *argv[]) {
   ...
   /* node 14 */
   findMax<<<gridsize, blocksize>>>(g_data, g_odata, N, instTrace0, numberOfThreads0);
   int numberOfThreads1 = 1 * blocksize.x * blocksize.y * blocksize.z;
   unsigned char *instTrace1;
   cudaMallocManaged(&instTrace1, numberOfThreads1 * 2000 * sizeof(unsigned char));
   /* node 15 */
   findMax<<<1, blocksize>>>(g_odata, d_c, limit, instTrace1, numberOfThreads1);
   /* node 16 */
   ...
   /* node 17 */
   findMin<<<gridsize, blocksize>>>(g_data, g_odata, N, instTrace2, numberOfThreads2);
   int numberOfThreads3 = 1 * blocksize.x * blocksize.y * blocksize.z;
   unsigned char *instTrace3;
   cudaMallocManaged(&instTrace3, numberOfThreads3 * 2000 * sizeof(unsigned char));
   /* node 18 */
   findMin<<<1, blocksize>>>(g_odata, d_c, limit, instTrace3, numberOfThreads3);
