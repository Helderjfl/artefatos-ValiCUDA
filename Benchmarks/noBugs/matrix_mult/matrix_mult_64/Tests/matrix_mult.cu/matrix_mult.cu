...
int main(int argc, char *argv[]) {
   ...
   for(/* node 17 */ i = 0; /* node 18 */ i < N; registerTraceHost(fpHost, 24), i++) {
      ...
      for(/* 20 */ j = 0; /* node 21 */ j < N; /* node 23 */ j++) {
         printf("%d ", h_c[i * N + j]);
   ...
   for(int threadIndex = 0; threadIndex < numberOfThreads0; threadIndex++)
   {
   	for(int nodeTrace = 0; nodeTrace < 2000; nodeTrace++)
   	{
   		if(instTrace0[threadIndex + nodeTrace * numberOfThreads0] != 0){
            ...
            //       instTrace0[threadIndex + nodeTrace * numberOfThreads0] != instTrace0[threadIndex + (nodeTrace-3) * numberOfThreads0] ||
            //       instTrace0[threadIndex + nodeTrace * numberOfThreads0] != instTrace0[threadIndex + (nodeTrace-6) * numberOfThreads0] ||
            //       instTrace0[threadIndex + (nodeTrace-1) * numberOfThreads0] != instTrace0[threadIndex + (nodeTrace-4) * numberOfThreads0] ||
            //       instTrace0[threadIndex + (nodeTrace-2) * numberOfThreads0] != instTrace0[threadIndex + (nodeTrace-5) * numberOfThreads0]){
            //          if(instTrace0[threadIndex + nodeTrace * numberOfThreads0] != instTrace0[threadIndex + (nodeTrace-2) * numberOfThreads0] ||
            //             instTrace0[threadIndex + nodeTrace * numberOfThreads0] != instTrace0[threadIndex + (nodeTrace-4) * numberOfThreads0] ||
            //             instTrace0[threadIndex + (nodeTrace-1) * numberOfThreads0] != instTrace0[threadIndex + (nodeTrace-3) * numberOfThreads0])
            //             {
            //             if(instTrace0[threadIndex + nodeTrace * numberOfThreads0] != instTrace0[threadIndex + (nodeTrace-1) * numberOfThreads0] ||
